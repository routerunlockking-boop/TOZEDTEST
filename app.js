...
// OPERATOR PASSWORD — from IMEI (v1 / standard)
...
function getS50LookupDB() {
  try {
    const stored = localStorage.getItem('s50_operator_pairs');
...
function operatorPassS50New(imei, mac) {
  ...
  // 2. Check user-added pairs from localStorage
...
function handleSubmit(e) {
  ...
  const opEl = document.getElementById('operatorVal');
  if (op || opNew) {
    let html = '';
    if (op) html += '<div class="op-version"><span class="op-pw">' + op + '</span> <span class="op-tag">v1</span></div>';
    if (opNew) html += '<div class="op-version s50new"><span class="op-pw">' + opNew + '</span> <span class="op-tag s50">S50 New</span></div>';
    opEl.innerHTML = html;
  } else {
    opEl.textContent = '—';
  }
  ...
  document.getElementById('operatorCard').classList.toggle('has-value', !!(op || opNew));
...
function copyVal(id) {
  let text;
  if (id === 'operatorVal') {
    // For operator, prefer S50 New if available, otherwise v1
    const el = document.getElementById(id);
    const s50div = el.querySelector('.s50new .op-pw');
    const v1div = el.querySelector('.op-pw');
    text = s50div ? s50div.textContent.trim() : (v1div ? v1div.textContent.trim() : el.textContent);
  } else {
...
function copyOpVersion(version) {
  const el = document.querySelector(version === 'v1' ? '.op-version:not(.s50new) .op-pw' : '.op-version.s50new .op-pw');
  if (!el) return;
  const text = el.textContent.trim();
  if (!text) return;
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text).then(() => flashCopy('operatorVal'));
  } else {
    const ta = document.createElement('textarea');
    ta.value = text; ta.style.cssText = 'position:fixed;opacity:0';
    document.body.appendChild(ta); ta.select();
    try { document.execCommand('copy'); flashCopy('operatorVal'); } catch {}
    document.body.removeChild(ta);
  }
}
